...
* monitor (console output) and publishes MQTT messages using specific topics. A Node Red program is 
* subscribed to all the topics that are published and it also sends the messages to Home Assistant. 
*
...
int WiFiLed = 2; //**********Pin 2 is the on-board blue LED********** (This is optional)
long lastMsg = 0;
byte mac[6];
...
// Checking MQTT status. If status is not connected, try to reconnect until a connection is established.
void checkingMqtt()
{  
  ...
  {
    mqttDisconnected++;
    client.connect(mqttClientID, mqttUser, mqttPassword, topicPrefix + "/lastWill", 0, 1, "Help, I lost connectiong.", true);
    Serial.print("Connecting to MQTT");
    for (int i = 0; i < 20; i++)
    {
      delay(1000);// One second delay
      Serial.print(".");
      client.connect(mqttClientID, mqttUser, mqttPassword, topicPrefix + "/lastWill", 0, 1, "Help, I'm having trouble connecting", true);
      if(client.connected())
      {
        break;
      }
...
// Printing to console the topic prefix (ex. home/room)
void printPrefix()
{   
...
// Calculating the the min humidity percent of the current day.
